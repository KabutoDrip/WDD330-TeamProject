...
import { removeProductFromCart, increaseProductQuantity, decreaseProductQuantity } from './productDetails.mjs';
...
function init() {
  ...
  const trashbtn = document.querySelectorAll('.cart-trash')
  trashbtn.forEach(btn => btn.addEventListener('click',(e) => {
  ...
  const decreases = document.querySelectorAll('.dec-quantity')
  decreases.forEach(btn => btn.addEventListener('click',(e) => {
    decreaseProductQuantity(e)
  },true))

  const increases = document.querySelectorAll('.inc-quantity')
  increases.forEach(btn => btn.addEventListener('click',(e) => {
...
function renderCartContents() {
  ...
  if(cartItems) {
    const quantity = cartItems.map((item) => initQuantity(item));
...
function cartItemTemplate(item) {
  ...
  <p class="cart-card__quantity">qty: ${item.CartQuantity}</p>
  <div class='item-qs'>
    <span class="dec-quantity">-</span>
      <input type="text" class="display" value='${item.CartQuantity}'>
    <span class="inc-quantity">+</span>
  </div>
